...
async def get_camera_frame(camera_id: int, db: Session = Depends(get_db)):
    ...
    # If capture failed momentarily, check last known good frame from camera service
    cache_key = camera_service._get_cache_key(
        camera_type=CameraTypeEnum(meta["camera_type"]),
        stream_url=meta["stream_url"],
        ip_address=meta["ip_address"],
    ...
    with camera_service._cache_lock:
        if cache_key in camera_service._last_known_good:
            _, last_res = camera_service._last_known_good[cache_key]
            if last_res.image_bytes:
                return Response(
                    content=last_res.image_bytes,
                    media_type="image/jpeg",
                    headers={"Cache-Control": "no-cache, no-store, must-revalidate, max-age=0"}
                )

    # Standby placeholder frame if never connected
    cam_display_title = meta.get("camera_name") or f"CCTV {camera_id}"
    standby_bytes = camera_service.generate_standby_frame(cam_display_title)
    return Response(
        content=standby_bytes,
        media_type="image/jpeg",
        headers={"Cache-Control": "no-cache, no-store, must-revalidate, max-age=0"}
    )
